...
if($grp != 0) {
    ...
    foreach ($grpusrs as $grpusr) { // loop to all students of  groups
        echo('      <tr onclick="document.location=\'view.php?id='.$cm->id.'&usrid='.$grpusr->id.'&groupid='.$grp->id.'&tab=2&results=3\'" style="cursor:pointer;" title="'.get_string('dynamoresults3', 'mod_dynamo').'">
    ...
    // NIWFS
    echo('        <tr>');
    echo('          <td style="background-color:dimgray;text-align:center;">'.get_string('dynamoniwf', 'mod_dynamo').'<a href="#" data-toggle="toolpit" dyna-data-title="'.get_string('dynamohelpniwf', 'mod_dynamo').'">&nbsp;<i class="fas fa-info-circle ico-black"></i></a>
    </td>');
    
    $i = count($aGridlib)-1;
    for($j=0;$j<count($aGridlib[$i]);$j++) {
        $niwf = $aGridlib[$i][$j];
        echo('        <td style="background-color:white;text-align:center;color:'.dynamo_get_color_niwf($niwf).'">'.number_format($niwf,2,',', ' ').'<br>'.(number_format(($niwf/$nbstudent)*100,2,',', ' ')).'&#37;</td>');
    }  
    echo('          </tr>');
    ...
    foreach ($grpusrs as $grpusr) {
        ...
        echo('<h4 class="group_detail_title" style="background-color:'.$facColor.'" title="'.get_string('dynamogotoparticipant', 'mod_dynamo').'" onclick="document.location=\'view.php?id='.$cm->id.'&usrid='.$grpusr->id.'&groupid='.$groupid.'&tab=2&results=3\'">'.$OUTPUT->render($avatar).' '.$grpusr->firstname.' '.$grpusr->lastname.'</h4>');
        ...
        echo(' </tbody>');
        echo('</table>');
        echo('<b>'.get_string('dynamoniwf', 'mod_dynamo').'</b> :<span style="padding:3px;border-radius:3px;color:white;background-color:'.dynamo_get_color_niwf($niwf[0]).'">'.number_format($niwf[0],2,',', ' ').'</span> <a href="#" data-toggle="toolpit" dyna-data-title="'.$niwf[1].'">&nbsp;<i class="fas fa-info-circle ico-blue"></i></a><br>');
